"""
v5에서 MLP Controller 추가.

Frequency-based Adaptive Image Normalization for Test-Time Adaptation (v6)

v6: Learnable MLP controller for adaptive histogram stretching parameters
    - Controller: g_φ(s) → (clip_low, clip_high, strength)
    - Hand-crafted prior as regularization anchor
    - Differentiable stretching operation
    - AdaptationEngine 통합 for TTA pipeline
"""
import torch
...
class FrequencyDegradationAnalyzer:
    ...
    Works on individual images without requiring domain statistics.
    """

    def __init__(self, cutoff_ratio: float = 0.1, device: str = 'cuda'):
    ...
    def compute_degradation(self, image: torch.Tensor) -> Tuple[float, float]:
        ...
        for c in range(C):
            ...
            # Create low-pass mask (circle in center)
            crow, ccol = H // 2, W // 2
            ...
            mask_low = ((x - ccol)**2 + (y - crow)**2 <= radius**2).float()

            fshift_low = fshift * mask_low
            fshift_high = fshift * (1 - mask_low)

            low_energy = torch.sum(torch.abs(fshift_low)**2)
            high_energy = torch.sum(torch.abs(fshift_high)**2)

            high_low_ratio = (high_energy / (low_energy + 1e-8)).item()
            high_low_ratios.append(high_low_ratio)

        avg_high_low_ratio = np.mean(high_low_ratios)
...
class FrequencyNormalizedModel(nn.Module):
    ...
    def __init__(self, base_model: nn.Module, cutoff_ratio: float = 0.1, device: str = 'cuda'):
        super().__init__()
        self.base_model = base_model
        self.analyzer = FrequencyDegradationAnalyzer(cutoff_ratio=cutoff_ratio, device=device)
        self.device = device
