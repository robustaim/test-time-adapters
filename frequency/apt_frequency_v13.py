"""
v13: Frequency-based self-supervised loss (TTT-Linear style)
- v12 기반 + 다중 ModelProvider 지원 (HuggingFace, Detectron2, Ultralytics)
...
class StretchingController(nn.Module):
    ...
    def get_prior(s: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # degradation_score=0 (clear): strength=0.1, low=5, high=95
        # degradation_score=1 (degraded): strength=0.5, low=1, high=99
        # v13: RT-DETR용으로 strength 줄임 (기존: 0.2 + 0.8 * s)
        strength_prior = 0.1 + 0.4 * s
...
class FrequencyTTAEngine(AdaptationEngine):
    ...
    def _forward_detectron2(self, batched_inputs):
        """Forward for Detectron2 models (list of dicts with 'image' key)."""
        ...
        self._compute_loss_and_update(all_stretched, all_params, all_priors)
    ...
    def _forward_huggingface(self, **kwargs):
        """Forward for HuggingFace models (pixel_values in kwargs)."""
        pixel_values = kwargs.get('pixel_values')
        if pixel_values is None:
            return self.base_model(**kwargs)

        # DEBUG: 실제 pixel_values 값 범위 확인
        # print(f"[DEBUG] pixel_values - min: {pixel_values.min():.3f}, max: {pixel_values.max():.3f}, mean: {pixel_values.mean():.3f}")
        ...
        if not self.adapting:
            stretched_list = []
            for i in range(B):
                stretched = self._normalize_image(pixel_values[i])
                stretched_list.append(stretched)
            new_pixel_values = torch.stack(stretched_list, dim=0)
            kwargs['pixel_values'] = new_pixel_values
            return self.base_model(**kwargs)
        ...
        for i in range(B):
            stretched, params, priors, s, original_scale = self._normalize_image(
                pixel_values[i], return_params=True
            )
            all_stretched.append(stretched)
            all_params.append(params)
            all_priors.append(priors)
            ...
            # For HuggingFace, output should match input scale
            if original_scale:
                stretched_for_model.append(stretched / 255.0)
            else:
                stretched_for_model.append(stretched)

        # Compute loss and update
        self._compute_loss_and_update(all_stretched, all_params, all_priors)

        # Stack and forward
        new_pixel_values = torch.stack([s.detach() for s in stretched_for_model], dim=0)
        kwargs['pixel_values'] = new_pixel_values
        return self.base_model(**kwargs)

    def _compute_loss_and_update(self, all_stretched, all_params, all_priors):
        """Compute frequency loss and update controller."""
        if not all_stretched:
            return
    ...
    def forward(self, *args, **kwargs):
        ...
        v13: 다중 ModelProvider 지원
        - Detectron2: batched_inputs (list of dicts)
        - HuggingFace: pixel_values in kwargs
        ...
        """
        provider = self.base_model.model_provider

        # HuggingFace: pixel_values in kwargs
        if provider == ModelProvider.HuggingFace:
            return self._forward_huggingface(**kwargs)

        # Detectron2: list of dicts as first arg
        if provider == ModelProvider.Detectron2:
            if len(args) > 0 and isinstance(args[0], list):
                return self._forward_detectron2(args[0])

        # Fallback: try Detectron2 style if args[0] is list
        if len(args) > 0 and isinstance(args[0], list):
            return self._forward_detectron2(args[0])

        # Otherwise pass through to base model
