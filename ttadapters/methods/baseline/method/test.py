...
class TeSTConfig(AdaptationConfig):
    adaptation_name: str = "TeSTEngine"
    model_type: Literal["rcnn", "swinrcnn", "rtdetr", "yolo11"] = "rcnn"
...
class TeSTEngine(AdaptationEngine):
    ...
    def __init__(self, base_model: BaseModel, config: TeSTConfig):
    ...
    def _apply_augmentation(self, batch):
        ...
        if self.config.model_type == "rtdetr":
        ...
        elif self.config.model_type in ("rcnn", "swinrcnn"):
    ...
    def _extract_pooled_backbone_features(self, model, batch) -> torch.Tensor:
        if self.config.model_type in ("rcnn", "swinrcnn"):
