...
class CascadedNormEngine(AdaptationEngine):
    ...
    def _maybe_reset_optimizer(self):
        """Reset optimizer state based on interval."""
        if self.config.optimizer_reset_interval <= 0:
            return

        if self.step_counter > 0 and self.step_counter % self.config.optimizer_reset_interval == 0:
            # Partial reset: Momentum only (exp_avg)
            # Keeps variance (exp_avg_sq) for stable adaptive scaling
